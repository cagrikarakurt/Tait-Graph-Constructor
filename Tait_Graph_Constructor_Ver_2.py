...
class GraphDrawer:
    def __init__(self, root):
        self.root = root
        self.root.title("Planar Multigrapher: Auto-Offset & Drag")
        ...
        self.status_label = tk.Label(root, text="VERTEX: Drag nodes to move | L-Click to add | 'D' to delete",
    ...
    def handle_press(self, event):
        ...
        elif mode == "EDGE" and node_idx is not None:
    ...
    def handle_release(self, event):
        if self.modes[self.current_mode_idx] != "ROUTE/EDIT":
            self.active_element = None
    ...
    def toggle_edge_type(self):
        if self.modes[self.current_mode_idx] == "ROUTE/EDIT" and self.active_element is not None:
            ...
            self.default_type = "-" if self.default_type == "+" else "+"
        self.refresh_canvas()
    ...
    def attempt_add_edge(self, u, v):
        ...
        # Auto-Offset logic for multigraphs
        pair = tuple(sorted((u, v)))
        existing_count = sum(1 for e in self.edges if tuple(sorted((e["u"], e["v"]))) == pair)
        ...
        if existing_count > 0:
            ...
            if dist > 0:
                magnitude = 45 * ((existing_count + 1) // 2)
                direction = -1 if existing_count % 2 == 0 else 1
                cx += (-dy / dist) * magnitude * direction
                cy += (dx / dist) * magnitude * direction
    ...
    def find_closest_edge_control(self, x, y):
        best_dist, best_idx = 25, None
        for i, e in enumerate(self.edges):
            d = math.hypot(x - e["cx"], y - e["cy"])
    ...
    def relax_graph(self):
        ...
        # Reset and Auto-fan control points after layout
        pair_counts = {}
        for e in self.edges:
            u, v = e["u"], e["v"]
            pair = tuple(sorted((u, v)))
